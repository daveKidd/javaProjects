...
public class Exercise04 {
    ...
    BigDecimal average(BigDecimal[] values) {
        ...
        return sum.divide(new BigDecimal(values.length), 4, RoundingMode.HALF_UP);
    }
    ...
    BigDecimal calculateInterest(BigDecimal investment, BigDecimal interestRate, int periods) {
        BigDecimal balance = new BigDecimal(investment.toString());

        for (int periodCount = 0; periodCount < periods; periodCount++) {
