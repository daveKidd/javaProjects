...
-- Expected: 68 Rows.
-- (Instructor Note: ifnull is not required.)
select *
from customer
where ifnull(phone, '') = '';
...
-- Expected: 4 Rows
select * from project_employee where project_id = 21;

-- Step 4: use the employee_ids from project_employee to find records in employee
-- Expected: 4 Rows
select * from employee where employee_id in (14, 18, 23, 32);