...
public class VideoGameJdbcTemplateRepository implements VideoGameRepository {
    ...
    public List<VideoGame> findAll() throws DataAccessException {
        final String sql = """
                select video_game_id, title, console, price, release_date, single_player
                from video_game;              
                """;
    ...
    public VideoGame findById(int videoGameId) throws DataAccessException {
        final String sql = """
                select video_game_id, title, console, price, release_date, single_player
                from video_game where video_game_id = ?;              
                """;
        return jdbcTemplate.query(sql,rowMapper,videoGameId).stream()
                .findFirst()
                .orElse(null);
        //return jdbcTemplate.queryForObject(sql,rowMapper,videoGameId);
    }
